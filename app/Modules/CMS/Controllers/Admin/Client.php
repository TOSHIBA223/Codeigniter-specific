...
class Client extends BaseController {
	...
	public function form($article_id = null)
	{
            ...
            if($this->validation->withRequest($this->request)->run()){
                ...
                if($this->request->getMethod() == "post"){
                    $image=$this->imagelibrary->image($img,$savepath,$old_image,200,200);
                }
            }
            ...
            if ($this->validation->withRequest($this->request)->run()){
                if (empty($article_id)){
                    ...
                    return  redirect()->to(base_url('backend/client/info'));   
                }else{   
                    $where = array( 
                    ...
                    return  redirect()->to(base_url('backend/client/info/'.$article_id)); 
                }
            }else{
            ...
            $data['content'] = $this->BASE_VIEW . '\article\client';
            return $this->template->admin_layout($data);
	...
	{  
		...
		$this->session->setFlashdata('exception', display('please_try_again'));
            ...
            return  redirect()->to(base_url('backend/client/client_list')); 
