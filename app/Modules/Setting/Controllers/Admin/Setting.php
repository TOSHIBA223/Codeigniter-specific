...
class Setting extends BaseController {
    ...
    public function commission_setup()
    {
            ...
            $data['setup_commission'] =$this->common_model->get_all('setup_commission',$where=array(),0,0,'level_name','ASC');
            $data['title'] = display('commission_setup');

            $data['content'] = $this->BASE_VIEW . '\commission_setting';
            return $this->template->admin_layout($data);
    ...
    public function commission_update()
    {
        ...
        $personal_invest = $this->request->getVar('personal_invest',FILTER_SANITIZE_STRING);
        $total_invest = $this->request->getVar('total_invest',FILTER_SANITIZE_STRING);
        $team_bonous = $this->request->getVar('team_bonous',FILTER_SANITIZE_STRING);
        $referral_bonous = $this->request->getVar('referral_bonous',FILTER_SANITIZE_STRING);
            ...
            for ($i=0; $i<=count($id); $i++) {
                        ...
                        $this->common_model->update('setup_commission',$where,$data);
                        }
            }
    
            $this->session->setFlashdata('message',display('update_successfully'));
            return  redirect()->to(base_url('backend/setting/commission_setup'));
    ...
    public function email_sms_setting()
    {
                ...
                $data['email']=$this->common_model->read('sms_email_send_setup',$whereemail);
                $wheresms=array(
                    'method' => 'sms'
                );
                $data['sms']=$this->common_model->read('sms_email_send_setup',$wheresms);
                $data['title'] = display('fees_setting');
                $data['content'] = $this->BASE_VIEW . '\email_and_sms_setting';
                return $this->template->admin_layout($data);
        
    }

        #----------------------------
        #Email and Sms setting Update
        #----------------------------
    ...
    public function update_sender()
    {
                ...
                $this->validation->setRule('payout', display('payout'),'numeric|permit_empty');
                $this->validation->setRule('commission', display('commission'),'numeric|permit_empty');
                $this->validation->setRule('team_bonnus', display('team_bonnus'),'numeric|permit_empty');
                if($this->validation->withRequest($this->request)->run())
                {
                    ...
                    if($email!=NULL){
                        $data = array(
                            ...
                            'payout' =>$this->request->getVar('payout',FILTER_SANITIZE_STRING),
                            'commission' =>$this->request->getVar('commission',FILTER_SANITIZE_STRING),
                            'team_bonnus' =>$this->request->getVar('team_bonnus',FILTER_SANITIZE_STRING)
                        );
                        $where=array(
                            'method'    =>$email
                        );
                        $this->common_model->update('sms_email_send_setup',$where,$data);
                        $this->session->setflashdata('message',display('update_successfully'));
                    }
                    if($sms!=NULL){
                        $data = array(
                ...
                return  redirect()->to(base_url('backend/setting/email_sms_setting'));
    ...
    {
            $data['title'] = "Emal/SMS template";
            $where=array(
                'id'    =>$id
            );
            $typecheck=$this->common_model->read('dbt_sms_email_template',$where);
            if($typecheck->sms_or_email != 'sms'){
                $this->validation->setRule('subject_en', display('Template EN title/subject'),'required|max_length[255]');
            }
            $this->validation->setRule('template_en', display('Template EN'),'required');
            $this->validation->setRule('template_fr', display('template_fr'),'required');
            
            $data['template']   = (object)$userdata = array(
                'id'        => $this->request->getVar('id',FILTER_SANITIZE_STRING),
                'subject_en'     => $this->request->getVar('subject_en',FILTER_SANITIZE_STRING),
                'subject_fr'     => $this->request->getVar('subject_fr',FILTER_SANITIZE_STRING),
                'template_en'    => $this->request->getVar('template_en',FILTER_SANITIZE_STRING),
                'template_fr'    => $this->request->getVar('template_fr',FILTER_SANITIZE_STRING)
            );
    ...
    public function sms_gateway()
    {
        ...
        $data['sms']        = $this->common_model->read('email_sms_gateway',$where); 
        $data['userrole']   = $this->setting_model->getMenuSingelRoleInfo(30);
        $data['content'] = $this->BASE_VIEW . '\sms_gateway';
         return $this->template->admin_layout($data);
    }
    ...
    public function update_sms_gateway()
    {
                ...
                $this->validation->setRule('title', display('title'),'required|max_length[50]');
               
        $sms = $this->request->getVar('es_id');
        $where =array(
                    'es_id' => 2
                );
        $pass = '';
                $password= $this->common_model->read('email_sms_gateway',$where);
        
        if($password->password == base64_decode($this->request->getVar('password',FILTER_SANITIZE_STRING))){
           $pass = $password->password;
        ...
        $data = array(
            'gatewayname'           =>$this->request->getVar('gatewayname',FILTER_SANITIZE_STRING),
            'title'         =>$this->request->getVar('title',FILTER_SANITIZE_STRING),
            'host'          =>$this->request->getVar('host',FILTER_SANITIZE_STRING),
            'user'          =>$this->request->getVar('user',FILTER_SANITIZE_STRING),
            'userid'        =>$this->request->getVar('userid'),
            'password'      =>$pass,
            'api'           =>$this->request->getVar('api',FILTER_SANITIZE_STRING)
        );
                //check validation for budgetsms
                if($data['gatewayname']=='budgetsms'){
                    $this->validation->setRule('host', display('host'),"required|max_length[50]");
                    $this->validation->setRule('user', display('user'),'required|max_length[50]');
                    $this->validation->setRule('userid', display('userid'),'required|max_length[50]');
                    $this->validation->setRule('api', display('api'),'required');
                }
                //check validation for infobip and Sms rank
                if($data['gatewayname']=='infobip' || $data['gatewayname']=='smsrank' ){
                    $this->validation->setRule('host', display('host'),"required|max_length[50]");
                    $this->validation->setRule('user', display('user'),'required|max_length[50]');
                    $this->validation->setRule('password', display('password'),'required|max_length[50]');
                   
                }
                //check validation for infobip
                if($data['gatewayname']=='nexmo'){
                    $this->validation->setRule('api', display('api'),'required');
                    $this->validation->setRule('password', display('password'),'required|max_length[50]'); 
                }
        ...
        return  redirect()->to(base_url('backend/setting/sms_gateway'));
    ...
    public function test_sms()
    {
            ...
            $this->validation->setRule('mobile_num', display('mobile_number'),'required');
            $this->validation->setRule('test_message', display('test_message'),'required');
            ...
            if($this->validation->withRequest($this->request)->run()){
                ...
                $mobile_num     = $this->request->getVar('mobile_num',FILTER_SANITIZE_STRING);
                $test_message   = $this->request->getVar('test_message',FILTER_SANITIZE_STRING);
                if ($mobile_num) {
                    try {
